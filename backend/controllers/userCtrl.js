...
const loginController = async (req, res) => {
  try {
    ...
    res.status(200).send({ 
      ...
      success: true, 
      token, 
      role: "teacher" // Add role to the response
    });
  } catch (error) {
    console.log(error);
...
const createClass = async (req, res) => {
  try {
    ...
    const teacherId = req.body.userId; // Extract teacherId from the request (set by authMiddleware)
    ...
    const newClassCode = new ClassCodeModel({
      code,
      teacherId, // Use the extracted teacherId
      className,
      section,
      subject,
      room,
