...
import { createMaterial, fetchMaterials } from "../../utils/api";// Ensure this path is correct

const Classwork = () => {
  const { id } = useParams(); // Get the class ID from the URL
  console.log("Class ID:", id); // Debugging: Check if `id` is correct
  ...
  useEffect(() => {
    const fetchClassMaterials = async () => {
      ...
      setError(null);
      try {
        const response = await fetchMaterials(id); // Ensure `id` is passed correctly
  ...
  const handleAddAssignment = async () => {
    ...
    formData.append("classId", id); // Ensure `id` is passed correctly
